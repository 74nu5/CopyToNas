...
var remotePathOption = new Option<string>(
    name: "--remote-path",
    description: "Chemin distant à copier")
{ IsRequired = true };

var localPathOption = new Option<string>(
    name: "--local-path",
    description: "Chemin local de destination")
{ IsRequired = true };
...
rootCommand.SetHandler(async (context) =>
{
    ...
    ConfigureSerilog(logLevel ?? "Information", enableFileLogging);
    ...
    {
        Log.Fatal(ex, "💀 Erreur critique lors de la copie SFTP");
        context.ExitCode = 1;
    }
});
